...
//		tabbedPane.addChangeListener(resizeDynamique());
    ...
    private JPanel createBottomButtonsPanel() {
        ...
        try {
            ...
            restoreButton.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent event) {
                    boolean yesPressed = Popup.yesNoCenterToComponent(ConfigWindow.getInstance(),"Are you sure you want to reset the properties values to those from the config file ?\n(those values may not be valid)");
                    if (yesPressed) {
    ...
    private void cancelButtonActionPerformed() {
        ...
        int userChoice = Popup.yesNoCloseCenterToComponent(ConfigWindow.getInstance(),"Continue with previous configuration (No = exit Proline Zero) ?");
    ...
    private void windowClosingActionPerformed() {
        ...
        if (configHasChanged) {
            ...
            int userChoice = Popup.yesNoCloseCenterToComponent(ConfigWindow.getInstance(),"Some modifications have been made do you want to save them? ");

            if (userChoice == JOptionPane.OK_OPTION) {
                configManager.updateConfigurationParams();

                int secondUserChoice = Popup.yesNoCloseCenterToComponent(ConfigWindow.getInstance(),"Modifications have been saved ,do you want to launch proline?");
            ...
            if (userChoice == JOptionPane.NO_OPTION) {
                ...
                int launchProline = Popup.yesNoCloseCenterToComponent(ConfigWindow.getInstance(),"Do you want to launch Proline with previous Configuration?");
            ...
            int userChoice = Popup.yesNoCloseCenterToComponent(ConfigWindow.getInstance(),"Do you want to launch proline with previous configuration?");
            if (userChoice == JOptionPane.OK_OPTION) {
                checkErrorAndExecute();
            }
            if (userChoice == JOptionPane.NO_OPTION) {
...
//	}
    ...
    private void updateModulesPanel() {
        doNotShowAgainBox.setSelected(!configManager.showConfigDialog());
        studioModuleBox.setSelected(configManager.isStudioActive());
        seqRepModuleBox.setSelected(configManager.isSeqRepActive());
    }
